...
const ProfilePage: React.FC = () => {
  ...
  const fetchRatingsCount = async () => {
    try {
      // Fetch first page to get all ratings and count them
      const response = await bidderApi.getReceivedRatings(1, 1000); // Large limit to get all
  ...
  const reputationPercentage =
    positiveCount + negativeCount > 0 ? Math.round((positiveCount / (positiveCount + negativeCount)) * 100) : 0;
