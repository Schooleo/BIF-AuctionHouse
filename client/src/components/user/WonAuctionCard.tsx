...
const WonAuctionCard: React.FC<WonAuctionCardProps> = ({ auction, onRate, onUpdateRating, onDeleteRating }) => {
  return (
    <div className='bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-all'>
      <div className='flex gap-3 p-3'>
        {/* IMAGE */}
        <Link to={`/product/${auction._id}`} className='shrink-0'>
        ...
        <div className='flex-1 min-w-0 flex flex-col justify-between'>
          <div className='space-y-1'>
            ...
            <div className='flex items-center gap-1 text-xs'>
              ...
              <span className='font-medium text-gray-700 truncate'>{auction.seller?.name || 'Unknown'}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className='space-y-2'>
            <div className='p-2 bg-gray-50 border border-gray-200 rounded-md'>
              <div className='flex items-center justify-between mb-1'>
                ...
                <span
                  className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${
                    auction.myRating?.score === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                  }`}
                >
              ...
              {auction.myRating?.comment && (
                <p className='text-xs text-gray-600 italic line-clamp-2'>"{auction.myRating.comment}"</p>
              )}
            </div>
