...
export interface IRating extends Document {
  ...
  product?: Types.ObjectId; // Sản phẩm (nếu có, transaction-based)
  score: 1 | -1; // +1 hoặc -1
  comment: string;
  createdAt: Date;
  updatedAt: Date;
...
const ratingSchema = new Schema<IRating>(
  {
    ...
    product: { type: Schema.Types.ObjectId, ref: "Product" }, // Required if type === 'bidder'
...
// Index unique:
// 1. Bidder đánh giá Seller: 1 lần duy nhất trên cặp (rater, ratee)
ratingSchema.index(
  { type: 1, rater: 1, ratee: 1 },
  { unique: true, partialFilterExpression: { type: "seller" } }
);

// 2. Seller đánh giá Bidder: 1 lần duy nhất trên mỗi giao dịch (rater, ratee, product)
ratingSchema.index(
  { type: 1, rater: 1, ratee: 1, product: 1 },
  { unique: true, partialFilterExpression: { type: "bidder" } }
);
...
async function updateUserReputation(doc: IRating, incrementBy: 1 | -1) {
  try {
    const fieldToUpdate =
      doc.score === 1 ? "positiveRatings" : "negativeRatings";
...
ratingSchema.post<IRating>(
  "findOneAndDelete",
  async function (doc: IRating | null) {
    if (doc) {
