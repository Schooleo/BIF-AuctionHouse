      ...
      if (isEnded && lastBidder) {
        if (scenarioType === 1) {
          ...
          if (scenarioType === 3) {
            ...
            // Randomly assign positive or negative rating (80% positive, 20% negative)
            const buyerScore = Math.random() > 0.2 ? 1 : -1;
            const buyerComment =
              buyerScore === 1
                ? 'This seller guy is awesome!'
                : 'Seller was slow to respond and item was not as described.';
            ...
            // Randomly assign positive or negative rating (80% positive, 20% negative)
            const sellerScore = Math.random() > 0.2 ? 1 : -1;
            const sellerComment =
              sellerScore === 1
                ? 'This buyer guy is awesome with fast payment!'
                : 'Payment was delayed and communication was poor.';
